...
import { ApplicationError, NotFound, ValidationError } from "@/lib/errors";
...
router.add(
  {
    ...
    schema: {
      body: Joi.object({
        // Atleast these three are required. May contain more
        email: Joi.string().email().required(),
        password: Joi.string().required(),
...
router.add(
  {
    ...
    middlewares: [authGuard()],
    // Note: skip schema validation for now
    // schema: {
    //   body: Joi.object({
  ...
  async (req, res) => {
    ...
    delete updates["creationMethod"];
    delete updates["oauthProfileId"];
